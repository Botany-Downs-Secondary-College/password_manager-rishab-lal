...
Rishab574 = ["the remy - papoose", "the room - oceanpool", "idk, come pretend site - pretend passwords"]
blacklist = []
userlist = ["Rishab574"]
...
while True:
    ...
    if account_question == "newaccount":
        print("if you want to go back to menu, enter command ['back_to_menu']")
            ...
            while True:
                while True:
                    ...
                    if len(password) >= 8:
                        ...
                        print("your password is less than 8 characters long, please select one that meets password requirements.")
    ...
    if name not in blacklist and device_status == "alg":
        while True:
            task = input("to see passwords type 1, or if adding a password add type 2, to log out type 3: ")
            try:
                task = int(task)
            except:
